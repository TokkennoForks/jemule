...
import static org.jmule.ui.UIConstants.*;
...
public class KadTab extends AbstractTab {
	...
	public KadTab(Composite shell, JMuleCore core) {		
		...
		routing_table_container.setText(Localizer.getString("mainwindow.kadtab.kad_nodes"));
		kad_tasks_container.setText(Localizer.getString("mainwindow.kadtab.kad_tasks"));
		...
		if (_core.getJKadManager().isConnected())
			setKadStatus.setText(Localizer.getString("mainwindow.kadtab.disconnect"));
		if (_core.getJKadManager().isConnecting())
			setKadStatus.setText(Localizer.getString("mainwindow.kadtab.stop_connecting"));
		if (_core.getJKadManager().isDisconnected())
			setKadStatus.setText(Localizer.getString("mainwindow.kadtab.connect"));
		...
		contact_list.addColumn(SWT.LEFT, KAD_CLIENT_ID_COLUMN_ID, Localizer.getString("mainwindow.kadtab.contact_list.column.contact_id"), Localizer.getString("mainwindow.kadtab.contact_list.column.contact_id.desc"), swtPreferences.getColumnWidth(KAD_CLIENT_ID_COLUMN_ID));
		contact_list.addColumn(SWT.LEFT, KAD_CLIENT_DISTANCE_COLUMN_ID, Localizer.getString("mainwindow.kadtab.contact_list.column.contact_distance"), Localizer.getString("mainwindow.kadtab.contact_list.column.contact_distance.desc"), swtPreferences.getColumnWidth(KAD_CLIENT_DISTANCE_COLUMN_ID));
			...
			public void contactAdded(final KadContact contact) {
				Display.getDefault().asyncExec(new Runnable() {
					public void run() {
						...
						routing_table_container.setText(Localizer.getString("mainwindow.kadtab.kad_nodes_number",contact_list.getItemCount()+""));
					}
		...
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_TYPE_COLUMN_ID, Localizer.getString("mainwindow.kadtab.task_list.column.task_type"), Localizer.getString("mainwindow.kadtab.task_list.column.task_type.desc"), swtPreferences.getColumnWidth(KAD_TASK_TYPE_COLUMN_ID));
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_LOOKUP_HASH_COLUMN_ID, Localizer.getString("mainwindow.kadtab.task_list.column.task_hash"), Localizer.getString("mainwindow.kadtab.task_list.column.task_hash.desc"), swtPreferences.getColumnWidth(KAD_TASK_LOOKUP_HASH_COLUMN_ID));
		kad_task_list.addColumn(SWT.LEFT, KAD_TASK_LOOKUP_INFO_COLUMN_ID, Localizer.getString("mainwindow.kadtab.task_list.column.task_info"), Localizer.getString("mainwindow.kadtab.task_list.column.task_info.desc"), swtPreferences.getColumnWidth(KAD_TASK_LOOKUP_INFO_COLUMN_ID));
	...
	private KadTask lookupTaskToKadTask(LookupTask task) {
		...
		if (task.getRequestType() == RequestType.FIND_NODE) {
			kad_task.task_type = Localizer.getString("mainwindow.kadtab.node_lookup");
		...
		if (task.getRequestType() == RequestType.FIND_VALUE) {
			kad_task.task_type = Localizer.getString("mainwindow.kadtab.keyword_search");
			Search search = _jkad.getSearch();
			SearchTask search_task = search.getSearchTask(task.getTargetID());
			if (search_task==null) return null; 
		...
		if (task.getRequestType() == RequestType.STORE) {
			kad_task.task_type = Localizer.getString("mainwindow.kadtab.store");
