...
public class SearchTab extends AbstractTab{
	...
	public SearchTab(Composite parent, JMuleCore core) {
				...
				SWTThread.getDisplay().asyncExec(new JMRunnable() {
					public void JMRun() {
						...
						if (tab != null) {
							...
							MainWindow.getLogger().fine(Localizer.getString("mainwindow.logtab.message_search_result_arrived",
		...
		label.setText(Localizer.getString("mainwindow.searchtab.label.search") + " : ");
		...
		search_button.setText(Localizer.getString("mainwindow.searchtab.button.search"));
		...
		adv_search.setText(Localizer.getString("mainwindow.searchtab.label.advanced"));
		...
		clear_advanced_options.setText(Localizer.getString("mainwindow.searchtab.label.clear"));
	...
	private void search() {
		if ((searchType.getSelectionIndex()==0)&&(!_core.getServerManager().isConnected())) {
			Utils.showWarningMessage(getShell(), Localizer.getString("mainwindow.searchtab.not_connected_to_server_title"), Localizer.getString("mainwindow.searchtab.not_connected_to_server"));
			return;
		}
		if ((searchType.getSelectionIndex()==1)&&(!_core.getJKadManager().isConnected())) {
			Utils.showWarningMessage(getShell(), Localizer.getString("mainwindow.searchtab.not_connected_to_kad_title"), Localizer.getString("mainwindow.searchtab.not_connected_to_kad"));
			return;
		}		
	...
	private void showAdvancedOptions() {
		...
		String text = "";
		if ((minFileSize != 0)||(maxFileSize!=0))
			text = " "+ Localizer.getString("mainwindow.searchtab.label.min_file_size")+" : "+FileFormatter.formatFileSize(minFileSize)+" "
			+ Localizer.getString("mainwindow.searchtab.label.max_file_size")+" : "+ FileFormatter.formatFileSize(maxFileSize)+" ";
		if (extension.length()!=0)
			text += Localizer.getString("mainwindow.searchtab.label.file_extension") + " : "+extension+" ";
		
		if (availableSources!=0)
			text += Localizer.getString("mainwindow.searchtab.label.file_availability") + " : " + availableSources+" ";
		
		if (completedSources!=0)
			text+= Localizer.getString("mainwindow.searchtab.label.file_sources") + " : "+completedSources + " ";
		if (fileType != FileType.ANY)
			text += Localizer.getString("mainwindow.searchtab.label.file_type") + " : " + fileType + " ";
		if (text.length()!=0) 
