...
public class NewWindow implements JMuleUIComponent {
	...
	public void initUIComponents() {
		if (type == WindowType.SHARED_DIR) {
			...
			dir_dialog.setText(Localizer.getString("newwindow.title.shared_dir"));
		...
		if	(type == WindowType.DOWNLOAD) {
			shell.setText(Localizer.getString("newwindow.title.download"));
			shell.setImage(SWTImageRepository.getMenuImage("folder_down.png"));
		}
		else {
			shell.setText(Localizer.getString("newwindow.title.server"));
		...
		learn_more_link.setText(Localizer.getString("newwindow.label.about_ed2k_links"));
		...
		learn_more_link.addMouseListener(new MouseAdapter() {	
				public void mouseDoubleClick(MouseEvent arg0) {
					...
					if (!Utils.launchProgram(path)) 
						Utils.showWarningMessage(shell, Localizer.getString("newwindow.error_open_url.title")
								, Localizer.getString("newwindow.error_open_url",path));
				}
		...
		button_paste_ed2k_link.setText(Localizer.getString("newwindow.button.paste_ed2k_link"));
		...
		button_clear.setText(Localizer.getString("newwindow.button.clear"));
		...
		button_ok.setText(Localizer.getString("newwindow.button.ok"));
		...
		button_cancel.setText(Localizer.getString("newwindow.button.cancel"));
	...
	private class DownloadList extends JMTable<ED2KFileLink> implements GenericTable {
		...
		public DownloadList(Composite composite) {
			...
			addColumn(SWT.LEFT, SWTConstants.NEW_DOWNLOAD_LIST_FILE_NAME_COLUMN_ID, Localizer.getString("newwindow.column.file_name"), Localizer.getString("newwindow.column.file_name.desc"), SWTPreferences.getInstance().getColumnWidth(SWTConstants.NEW_DOWNLOAD_LIST_FILE_NAME_COLUMN_ID));
			addColumn(SWT.LEFT, SWTConstants.NEW_DOWNLOAD_LIST_FILE_SIZE_COLUMN_ID, Localizer.getString("newwindow.column.file_size"), Localizer.getString("newwindow.column.file_size.desc"), SWTPreferences.getInstance().getColumnWidth(SWTConstants.NEW_DOWNLOAD_LIST_FILE_SIZE_COLUMN_ID));
			addColumn(SWT.LEFT, SWTConstants.NEW_DOWNLOAD_LIST_FILE_ID_COLUMN_ID,   Localizer.getString("newwindow.column.file_id"), Localizer.getString("newwindow.column.file_id.desc"), 	   SWTPreferences.getInstance().getColumnWidth(SWTConstants.NEW_DOWNLOAD_LIST_FILE_ID_COLUMN_ID));
			setSorting(false);
			...
			paste_ed2k.setText(Localizer.getString("newwindow.menu.paste_ed2k_link"));
	...
	private class ServerList extends JMTable<ED2KServerLink> implements GenericTable {
		...
		public ServerList(Composite composite) {
			super(composite, SWT.CHECK);
			addColumn(SWT.LEFT, SWTConstants.NEW_SERVER_LIST_NAME_COLUMN_ID, Localizer.getString("newwindow.column.server_name"), Localizer.getString("newwindow.column.server_name.desc"), SWTPreferences.getInstance().getColumnWidth(SWTConstants.NEW_SERVER_LIST_NAME_COLUMN_ID));
			addColumn(SWT.LEFT, SWTConstants.NEW_SERVER_LIST_PORT_COLUMN_ID, Localizer.getString("newwindow.column.server_port"), Localizer.getString("newwindow.column.server_port"), SWTPreferences.getInstance().getColumnWidth(SWTConstants.NEW_SERVER_LIST_PORT_COLUMN_ID));
