...
public class UpdaterWindow implements JMuleUIComponent {
	...
	public void initUIComponents() {
		...
		shell.setText(Localizer.getString("updaterwindow.title"));
		...
		label.setText(JMConstants.JMULE_NAME + " " + Localizer.getString("updaterwindow.label.jmule_version") + " : ");
		...
		label = new Label(window_content,SWT.NONE);
		label.setFont(skin.getDefaultFont());
		label.setText(Localizer.getString("updaterwindow.label.available_version") + " : ");
		label.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END));
		...
		label.setFont(skin.getDefaultFont());
		label.setForeground(skin.getDefaultColor());
		label.setText(Localizer.getString("updaterwindow.label.last_update") + " : ");
		label.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END));
		...
		download_link.setText(Localizer.getString("updaterwindow.label.download_link"));
		...
		download_link.addMouseListener(new MouseAdapter() {	
			public void mouseDoubleClick(MouseEvent arg0) {
				...
				if (!Utils.launchProgram(path)) 
					Utils.showWarningMessage(shell, Localizer.getString("updaterwindow.error_open_url.title")
							, Localizer.getString("updaterwindow.error_open_url",path));
			}
		...
		changelog_group.setText(Localizer.getString("updaterwindow.group.changelog"));
		...
		button_startup_check.setText(Localizer.getString("updaterwindow.check.startup_check"));
		...
		button_ok.setText(Localizer.getString("updaterwindow.button.ok"));
		...
		new JMThread(new JMRunnable() {
			public void JMRun() {
				...
				try {
					...
					SWTThread.getDisplay().asyncExec(new JMRunnable() {
						public void JMRun() {
							Utils.showWarningMessage(shell, Localizer.getString("updaterwindow.connect_failed_title"), Localizer.getString("updaterwindow.connect_failed")+"\n"+updater.getErrorCode());
	...
	private void setWindowStatus(WindowStatus status) {
		if (status == WindowStatus.CONNECTING) {
			...
			available_version.setText(Localizer.getString("updaterwindow.label.connecting"));
		...
		if (status == WindowStatus.FAILED) {
			changelog_text.setText("");
			available_version.setText(Localizer.getString("updaterwindow.label.failed"));
